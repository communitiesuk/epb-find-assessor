      ...
      },
      {
        ...
        test:
          lambda do |adapter|
            ...
            if !walls.nil? &&
                !walls.compact.select { |desc| desc.downcase == "wall" }.empty?
              return false
            end

            roofs = method_or_nil(adapter, :all_roof_descriptions)
            if !roofs.nil? &&
                !roofs.compact.select { |desc| desc.downcase == "roof" }.empty?
              return false
            end

            floors = method_or_nil(adapter, :all_floor_descriptions)
            if !floors.nil? &&
                !floors.compact.select { |desc|
                  desc.downcase == "floor"
                }.empty?
              return false
            end
            ...
            if !main_heating.nil? &&
                !main_heating.compact.select { |desc|
            ...
            if !main_heating_controls.nil? &&
                !main_heating_controls.compact.select { |desc|
            ...
            if !secondary_heating.nil? &&
                !secondary_heating.compact.select { |desc|
